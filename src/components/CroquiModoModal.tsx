...
function PieceStep({ onBack, onSelect }: { onBack: () => void; onSelect: (peca: ReferencePiece) => void }) {
  return <><div className="flex items-center justify-between border-b border-white/10 pb-3"><button onClick={onBack} className="text-white/60 flex items-center gap-1.5 text-xs font-semibold uppercase"><ArrowLeft size={16} /> Voltar</button><span className="text-xs font-bold text-[#E5D3A2] uppercase tracking-widest">Tipo da peça</span><div className="w-12" /></div><div className="py-2 space-y-4"><p className="text-sm text-white/70 max-w-xs mx-auto">Qual tipo de roupa o GPT-5.4 mini deve procurar no recorte?</p><p className="text-[10px] text-white/45 max-w-xs mx-auto">Essa escolha orienta o foco da IA. Para enviar duas fotos (cima + baixo), escolha Vestido.</p><div className="grid grid-cols-2 gap-2">{REFERENCE_PIECES.map((peca) => <button key={peca} onClick={() => onSelect(peca)} className="p-3 rounded-xl bg-white/5 border border-white/15 text-xs font-bold text-white uppercase hover:border-[#E5D3A2]/60">{peca}</button>)}</div></div></>;
}
