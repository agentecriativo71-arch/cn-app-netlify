...
export function CroquiModoModal({ open, nome, onClose, onSelectCriarDoZero }: CroquiModoModalProps) {
  ...
  return <div className="modal-overlay z-50" onClick={(event) => event.target === event.currentTarget && onClose()}><div className="modal-card max-w-lg space-y-6 text-center select-none fade-in">
    ...
    {step === "analysis" && analysis && <AnalysisStep analysis={analysis} status={sessionStatus} confirming={confirming} error={modalError} onConfirm={confirmAnalysis} onRecrop={requestRecrop} onRetry={retryGeneration} />}
...
function AnalysisStep({ analysis, status, confirming, error, onConfirm, onRecrop, onRetry }: { analysis: ReferenceAnalysis; status: string; confirming: boolean; error: string | null; onConfirm: () => void; onRecrop: () => void; onRetry: () => void }) {
  const item = (label: string, observation: { value: unknown; confidence: number; evidence: string | null }) => {
    const value = typeof observation.value === "string" ? observation.value : observation.value === false ? "Sem manga" : observation.value === true ? "Confirmado" : "Não identificado";
    return <div className="border-b border-white/10 py-2 text-left"><div className="flex justify-between gap-3"><span className="text-white/50 text-xs">{label}</span><span className="text-white text-xs font-semibold">{value}</span></div><div className="mt-1 flex justify-between gap-2 text-[10px] text-white/40"><span>{observation.evidence || "Sem evidência registrada."}</span><span className="shrink-0 text-[#E5D3A2]">{Math.round(observation.confidence * 100)}%</span></div></div>;
  };
  const manga = analysis.possuiManga.value === false ? { ...analysis.possuiManga, value: "Sem manga" } : analysis.manga;
  const renda = analysis.rendaDecisao.value === true ? analysis.renda : { ...analysis.rendaDecisao, value: "Não confirmada" };
  const blocked = status === "needs_recrop" || status === "unsupported_garment";
  return <><div className="flex items-center justify-between border-b border-white/10 pb-3"><span className="text-xs font-bold text-[#E5D3A2] uppercase tracking-widest">Resumo da análise</span><span className="text-[10px] text-white/40">GPT-5 Vision</span></div><div className="space-y-2 text-xs text-white/70">{analysis.focus.map((focus) => <p key={focus.role}><span className="text-white/50 uppercase">Foco {focus.role}:</span> {focus.targetDescription || "peça principal"}<br /><span className="text-white/50">Evidência: {focus.evidence || "Sem evidência registrada."} · {Math.round(focus.confidence * 100)}%</span></p>)}</div><div className="space-y-0">{item("Peça", analysis.peca)}{item("Comprimento", analysis.comprimento)}{item("Decote", analysis.decote)}{item("Manga", manga)}{item("Saia", analysis.saia)}{item("Renda", renda)}</div>{error && <p className="text-xs text-red-300">{error}</p>}{blocked ? <><p className="text-xs text-amber-200">{status === "unsupported_garment" ? "A peça não corresponde a um tipo de roupa suportado." : "O foco ou a confiança ainda não são suficientes."} Faça um novo recorte no celular.</p><button onClick={onRecrop} className="w-full py-3 rounded-xl border border-[#E5D3A2]/50 text-[#E5D3A2] text-xs font-bold uppercase">Solicitar novo recorte</button></> : status === "generation_failed" ? <button onClick={onRetry} disabled={confirming} className="w-full py-3 rounded-xl bg-[#E5D3A2] text-black text-xs font-bold uppercase">{confirming ? "Tentando..." : "Tentar gerar novamente"}</button> : <div className="grid grid-cols-2 gap-3"><button onClick={onRecrop} disabled={confirming} className="py-3 rounded-xl border border-white/20 text-white text-xs font-bold uppercase">Refazer recorte</button><button onClick={onConfirm} disabled={confirming || status === "generating"} className="py-3 rounded-xl bg-[#E5D3A2] text-black text-xs font-bold uppercase">{confirming || status === "generating" ? "Gerando..." : "Confirmar e gerar"}</button></div>}</>;
}
