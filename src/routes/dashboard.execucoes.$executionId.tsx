...
import { getDashboardSpecificationEntries, getExecutionDetailFn } from "@/server/dashboardApi";
...
function ExecutionDetailPage() {
  ...
  const specificationEntries = getDashboardSpecificationEntries(detail.specification);
  return (
    <section>
      ...
      <div className="grid lg:grid-cols-2 gap-5">
        <div className="space-y-5">
          <div className="card-soft p-5">
            <h3 className="font-semibold text-white mb-3">Seleção do usuário</h3>
            {specificationEntries.length === 0 ? (
              <p className="text-sm text-white/55">Nenhuma seleção foi registrada nesta execução.</p>
            ) : (
              <dl className="divide-y divide-white/10">
                {specificationEntries.map((entry) => <div key={entry.key} className="flex justify-between gap-4 py-2 text-sm"><dt className="text-white/60">{entry.label}</dt><dd className="text-white text-right break-words">{entry.value}</dd></div>)}
              </dl>
            )}
          </div>
        ...
        <div className="card-soft p-5"><h3 className="font-semibold text-white mb-3">Artefatos e avaliações</h3>{detail.artifacts.length === 0 ? <p className="text-sm text-white/55">Nenhum artefato foi persistido nesta execução.</p> : <div className="grid sm:grid-cols-2 gap-3">{detail.artifacts.map((artifact) => <article key={artifact.id} className="rounded-lg border border-white/10 p-3"><p className="text-sm text-white">{artifact.kind}{artifact.selected ? " · aprovado" : " · reprovado ou não selecionado"}</p><p className="text-xs text-white/55">{artifact.status} · retenção até {new Date(artifact.retentionUntil).toLocaleDateString("pt-BR")}</p><p className="text-xs text-white/75 mt-1">Nota: {artifact.rating == null ? "não avaliado" : `${artifact.rating}/5`}</p>{typeof artifact.metadata.score === "number" && <p className="text-xs text-white/55">Score Vision: {artifact.metadata.score}</p>}{typeof artifact.metadata.seed === "number" && <p className="text-xs text-white/55">Seed: {artifact.metadata.seed}</p>}{Array.isArray(artifact.metadata.rejectionReasons) && artifact.metadata.rejectionReasons.length > 0 && <p className="text-xs text-red-200 mt-1">Reprovação: {artifact.metadata.rejectionReasons.join(", ")}</p>}{artifact.signedUrl ? <><img className="mt-3 rounded-md max-h-56 w-full object-contain bg-black/20" src={artifact.signedUrl} alt={`Artefato ${artifact.kind}`} /><a className="inline-block mt-2 text-xs text-[#E5D3A2] underline break-all" href={artifact.signedUrl} target="_blank" rel="noreferrer">Abrir URL assinada da imagem</a></> : <p className="text-xs text-amber-200 mt-2">URL indisponível: o arquivo não está disponível no Storage privado.</p>}</article>)}</div>}{detail.notifications.length > 0 && <p className="text-xs text-white/65 mt-4">Telegram: {detail.notifications.map((notification) => `${notification.status} (${notification.attempts} tentativa${notification.attempts === 1 ? "" : "s"})`).join(" · ")}</p>}</div>
      </div>
    </section>
  );
}
