  ...
  return (
    <div className="min-h-screen bg-[#121212] text-white flex flex-col justify-between p-4 sm:p-6 select-none max-w-md mx-auto">
      ...
      <main className="flex-1 flex flex-col items-center justify-center py-4 space-y-4">
        ...
        <p className="text-[10px] leading-relaxed text-center text-white/45 max-w-sm">Privacidade: a foto original permanece neste aparelho. O recorte principal enviado é anonimizado (rosto ocultado e fundo neutralizado) e fica retido por até 30 dias para permitir uma nova geração. Os três detalhes derivados são usados somente durante a análise e não são armazenados.</p>
...
function SingleSlot({ preview, onChange, onRecrop }: { preview: string | null; onChange: (event: React.ChangeEvent<HTMLInputElement>) => void; onRecrop: () => void }) {
  ...
  return <div className="w-full space-y-3"><PreviewImage src={preview} alt="Recorte anonimizado do modelo" onRecrop={onRecrop} onChange={onChange} /><p className="text-[11px] text-center text-white/60">Este recorte anonimizado e três detalhes derivados serão enviados à {CRISPIM_AI_LABEL}; somente o recorte principal fica retido por até 30 dias.</p></div>;
}
